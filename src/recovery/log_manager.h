...
static std::string LogTypeStr[] = {
    "UPDATE", "INSERT", "DELETE", "BEGIN", "COMMIT", "ABORT"};
...
public:
    ...
    virtual void format_print() {
        std::cout << "log type in father_function: "
                  << LogTypeStr[static_cast<int>(log_type_)] << "\n";
        printf("Print Log Record:\n");
        printf("log_type_: %s\n", LogTypeStr[static_cast<int>(log_type_)].c_str());
        printf("lsn: %d\n", lsn_);
...
public:
    ...
    explicit BeginLogRecord(txn_id_t txn_id)
        : BeginLogRecord() {
        log_tid_ = txn_id;
    }
    // 序列化Begin日志记录到dest中
    ...
    virtual void format_print() override {
        std::cout << "log type in son_function: "
                  << LogTypeStr[static_cast<int>(log_type_)] << "\n";
...
class LogManager {
public:
    explicit LogManager(std::shared_ptr<DiskManager> disk_manager)
