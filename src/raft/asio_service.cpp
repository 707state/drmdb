...
asio_service::meta_cb_params req_to_params(ptr<req_msg>& req) {
    return {static_cast<int>(req->get_type()),
            req->get_src(),
            req->get_dst(),
            req->get_term(),
...
public:
    void prepare_handshake() {
        // this is safe since we only expose ctor to new_ptr
    ...
    void handle_handshake(ptr<rpc_session> self, const ERROR_CODE& err) {
        if (!err) {
            ...
            ptr<asio::steady_timer> timer =
                new_ptr<asio::steady_timer>(std::ref(impl_->get_io_svc()));
...
private:
    ...
    void read_complete(ptr<buffer> hdr, ptr<buffer> log_ctx) {
        ...
        try {
            ...
            std::string meta_str;
            ptr<req_msg> req =
                new_ptr<req_msg>(term, t, src, dst, last_term, last_idx, commit_idx);
            if (hdr->get_int() > 0 && log_ctx) {
                ...
                while (log_ctx_size > ss.pos()) {
                    ...
                    ss.get_buffer(buf);
                    ptr<log_entry> entry(
                        new_ptr<log_entry>(term, buf, val_type, timestamp));
...
private:
    void start() {
        ...
        ptr<rpc_session> session = new_ptr<rpc_session>(session_id_cnt_.fetch_add(1),
                                                        impl_,
                                                        std::ref(io_svc_),
                                                        std::ref(ssl_ctx_),
                                                        ssl_enabled_,
...
public:
    asio_rpc_client(asio_service_impl* _impl,
        ...
        , logger_(l) {
        client_id_ = _impl->assign_client_id();
        if (ssl_enabled_) {
#ifdef SSL_LIBRARY_NOT_FOUND
            assert(0); // Should not reach here.
#else
            ...
            ssl_socket_.set_verify_callback([this](auto&& PH1, auto&& PH2) {
                return verify_certificate(std::forward<decltype(PH1)>(PH1),
...
public:
    ...
    void send_retry(ptr<asio_rpc_client> self,
        ...
        if (err || num_send_fails_ >= SEND_RETRY_MAX) {
            ...
            abandoned_ = true;
            ptr<resp_msg> rsp;
            ptr<rpc_exception> except(new_ptr<rpc_exception>(
    ...
    virtual void send(ptr<req_msg>& req,
        ...
        for (auto& entry: req->log_entries()) {
            ...
            ptr<buffer> entry_buf = buffer::alloc(LOG_ENTRY_SIZE + le->get_buff().size());
#if 0
            ...
            ss.put_i32(le->get_buff().size());
            ss.put_raw(le->get_buff().data_begin(), le->get_buff().size());
#endif
...
private:
    ...
    void response_read(ptr<req_msg>& req,
        ...
        ptr<resp_msg> rsp(new_ptr<resp_msg>(
...
asio_service_impl::asio_service_impl(const asio_service::options& _opt, ptr<logger> l)
    ...
    if (my_opt_.enable_ssl_) {
        ...
        // For client
        ssl_client_ctx_.load_verify_file(_opt.root_vert_file_);
#endif
    }
    ...
    for (unsigned int i = 0; i < cpu_cnt; ++i) {
        ptr<std::thread> t =
            new_ptr<std::thread>(std::bind(&asio_service_impl::worker_entry, this));
...
ptr<rpc_client> asio_service::create_client(const std::string& endpoint) {
    ...
    return new_ptr<asio_rpc_client>(impl_,
                                    std::ref(impl_->io_svc_),
                                    std::ref(impl_->ssl_client_ctx_),
                                    hostname,
                                    port,
...
ptr<rpc_listener> asio_service::create_rpc_listener(ushort listening_port,
                                                    ptr<logger>& l) {
    try {
        return new_ptr<asio_rpc_listener>(impl_,
                                          std::ref(impl_->io_svc_),
                                          std::ref(impl_->ssl_server_ctx_),
                                          listening_port,
                                          impl_->my_opt_.enable_ssl_,
                                          l);
    } catch (std::exception& ee) {
...
ptr<asio_service> raft_global_mgr::init_asio_service(const asio_service_options& asio_opt,
                                                     ptr<logger> logger_inst) {
    raft_global_mgr* mgr = get_instance();
    if (!mgr) return nullptr;

    std::lock_guard<std::mutex> l(mgr->asio_service_lock_);
    if (mgr->asio_service_) return mgr->asio_service_;

    mgr->asio_service_ = new_ptr<asio_service>(asio_opt, logger_inst);
    return mgr->asio_service_;
}

ptr<asio_service> raft_global_mgr::get_asio_service() {
    // NOTE:
