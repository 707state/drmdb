...
namespace raft {
buffer_serializer::buffer_serializer(buffer& src_buf, buffer_serializer::endiances endian)
    : endian_(endian)
    , buf_(src_buf)
    , pos_(0) {}

buffer_serializer::buffer_serializer(ptr<buffer>& src_buf_ptr,
                                     buffer_serializer::endiances endian)
    : endian_(endian)
    , buf_(*src_buf_ptr)
    , pos_(0) {}
...
void* buffer_serializer::data() const {
    auto* ptr = (uint8_t*)buf_.data_begin();
    return ptr + pos();
}
