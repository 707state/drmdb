...
public:
    peer(ptr<srv_config>& config,
        ...
        , max_heartbeat_interval_(ctx.get_params()->max_heartbeat_interval())
        , next_log_idx_(0)
...
public:
    ...
    void set_next_batch_size_hint_in_bytes(int64 batch_size) {
        next_batch_size_hint_in_bytes_.store(batch_size);
    }
    ...
    void slow_down_heartbeat() {
        current_heartbeat_interval_ =
            std::min(max_heartbeat_interval_, current_heartbeat_interval_ + rpc_backoff_);
    }
    void resume_heartbeat_speed() { current_heartbeat_interval_ = heartbeat_interval_; }
    void set_heartbeat_interval(int32 new_interval) {
        heartbeat_interval_ -= new_interval;
    }
    ...
    int32 get_heartbeat_cnt_since_leave() const {
        return heartbeat_cnt_since_leave_.load();
    }
