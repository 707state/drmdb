...
int32 raft_server::get_snapshot_sync_block_size() const {
    int32 block_size = ctx_->get_params()->snap_shot_block_size_;
...
ptr<req_msg> raft_server::create_sync_snapshot_req(ptr<peer>& pp,
    ...
    if (!snp || sync_ctx->get_offset() == 0) {
        ...
        ulong snp_timeout_ms = ctx_->get_params()->heartbeat_interval_
    ...
    if (params->use_bg_thread_for_snapshot_io_) {
        ...
        snapshot_io_mgr::get_instance().push(
    ...
    req->log_entries().push_back(
        new_ptr<log_entry>(term, sync_req->serialize(), log_val_type::snp_sync_req));
...
ptr<resp_msg> raft_server::handle_install_snapshot_req(req_msg& req) {
    ...
    ptr<snapshot_sync_req> sync_req =
        snapshot_sync_req::deserialize(entries[0]->get_buff());
...
void raft_server::handle_install_snapshot_resp(resp_msg& resp) {
    ...
    auto it = peers_.find(resp.get_src());
    if (it == peers_.end()) {
    ...
    try {
        ...
        if (is_last_obj) {
            ...
            if (log_store_->compact(req.get_snapshot().get_last_log_idx())) {
                ...
                logging_sm_target_index_ = req.get_snapshot().get_last_log_idx();

                ctx_->state_mgr_->save_state(*state_);

                ptr<snapshot> new_snp =
                    new_ptr<snapshot>(req.get_snapshot().get_last_log_idx(),
                                      req.get_snapshot().get_last_log_term(),
                                      c_conf,
