...
#include "logger.h"
#include "raft/debugging_options.h"
...
ptr<resp_msg> raft_server::handle_cli_req(req_msg& req,
    ...
    resp = new_ptr<resp_msg>(
        cur_term, msg_type::append_entries_response, id_, std::ref(leader_));
    ...
    for (size_t i = 0; i < num_entries; ++i) {
        ...
        if (ext_params.after_precommit_) {
            ...
            cb_params.log_idx_ = last_idx;
            cb_params.log_term_ = cur_term;
    ...
    if (!get_config()->is_async_replication()) {
...

        {
            ...
            auto entry = coommit_ret_elems_.find(last_idx);
            if (entry != coommit_ret_elems_.end()) {
                ...
                coommit_ret_elems_.insert(std::make_pair(last_idx, elem));
            }
...
void raft_server::drop_all_pending_commit_elems() {
    ...
    if (ctx_->get_params()->return_method_type_ == raft_params::blocking) {
        auto_lock(commit_ret_elems_lock_);
        ulong min_idx = std::numeric_limits<ulong>::max();
        ulong max_idx = 0;
        for (auto& entry: coommit_ret_elems_) {
            ptr<commit_ret_elem>& elem = entry.second;
            elem->ret_value_ = nullptr;
            elem->result_code_ = cmd_result_code::CANCELED;
    ...
    for (auto& entry: elems) {
        ...
        ee->async_result_->set_result(result, err, cmd_result_code::CANCELED);
    }
}
