...
raft_server::raft_server(context* ctx, const init_options& opt)
    ...
    if (params->enable_randomized_snap_creatioin_ && !get_last_snapshot()
    ...
    logging_sm_target_index_ = log_store_->next_slot() - 1;
    ...
    for (cluster_config::srv_iter it = srvs.begin(); it != srvs.end(); ++it) {
            ...
            peers_.insert(
                std::make_pair(cur_srv->get_id(),
                               new_ptr<peer,
                                       ...
                                       ptr<logger>&>(cur_srv, *ctx_, exec, logger_)));
        } else {
            // Myself.
...
void raft_server::start_server(bool skip_initial_election_timeout) {
    ptr<raft_params> params = ctx_->get_params();
    raft_global_mgr* mgr = raft_global_mgr::get_instance();
    if (mgr) {
...
void raft_server::update_params(const raft_params& new_params) {
    ...
    for (auto& entry: peers_) {
        ...
        p->set_heartbeat_interval(clone->heartbeat_interval_);
        p->resume_heartbeat_speed();
    }
}
...
void raft_server::apply_and_log_current_params() {
    ...
    if (!test_mode_flag_) {
        if (params->heartbeat_interval_ >= params->election_timeout_lower_bound_) {
            params->election_timeout_lower_bound_ = params->heartbeat_interval_ * 2;
    ...
    p_in("parameters: "
         ...
         params->parrallel_log_appending_ ? "ON" : "OFF");

    status_check_timer_.set_duration_ms(params->heartbeat_interval_);
...
void raft_server::shutdown() {
    ...
    if (params->use_bg_thread_for_snapshot_io_) {
        snapshot_io_mgr::get_instance().drop_reqs(this);
    }
...
int32 raft_server::get_quorum_for_commit() {
    ...
    if (params->exclude_snp_receiver_from_quorom_) {
...
int32 raft_server::get_leadership_expiry() {
    ...
    if (expiry == 0) {
        ...
        expiry =
            params->heartbeat_interval_ * raft_server::raft_limits_.leadership_limit_;
    }
    return expiry;
}
...
size_t raft_server::get_not_responding_peers() {
    ...
    ptr<raft_params> params = ctx_->get_params();
    int expiry = params->heartbeat_interval_ * raft_server::raft_limits_.response_limit_;
...
ptr<resp_msg> raft_server::process_req(req_msg& req, const req_ext_params& ext_params) {
    ...
    if (req.get_type() == msg_type::append_entries_request
        ...
        if (!im_learner_ && !heartbeat_alive_ && term_updated
            && req.get_type() == msg_type::request_vote_request) {
...
void raft_server::become_leader() {
...

    {
        ...
        p_in("number of pending commit elements: %zu", coommit_ret_elems_.size());
    }
    ...
    ptr<raft_params> params = ctx_->get_params();
    {
        ...
        for (ulong ii = s_idx; ii < e_idx; ++ii) {
            ...
            p_in("found uncommitted config at %zu, size %zu", ii, le->get_buff().size());
            last_config = cluster_config::deserialize(le->get_buff());
        }
...
void raft_server::check_leadership_transfer() {
    ...
    size_t hb_interval_ms = ctx_->get_params()->heartbeat_interval_;
...
void raft_server::yield_leadership(bool immediate_yield, int successor_id) {
    ...
    reelaction_timer_.set_duration_ms(ctx_->get_params()->election_timeout_upper_bound_);
    reelaction_timer_.reset();
}
...
void raft_server::become_follower() {
    ...
    p_in("[BECOME FOLLOWER] term %lu", state_->get_term());
    {
        ...
        for (auto it = peers_.begin(); it != peers_.end(); ++it) {
            it->second->enable_heartbeat(false);
        }
...
void raft_server::handle_ext_resp_err(rpc_exception& err) {
    ...
    if (p->get_current_heartbeat_interval()
        >= ctx_->get_params()->max_heartbeat_interval()) {
        ...
        schedule_task(task, p->get_current_heartbeat_interval());
    }
}
