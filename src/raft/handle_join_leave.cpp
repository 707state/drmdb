...
ptr<resp_msg> raft_server::handle_add_srv_req(req_msg& req) {
    ...
    ptr<resp_msg> resp = new_ptr<resp_msg>(
        state_->get_term(), msg_type::add_server_response, id_, std::ref(leader_));
    ...
    ptr<srv_config> srv_conf = srv_config::deserialize(entries[0]->get_buff());
    ...
    if (srv_to_join_) {
        ...
        if (last_active_ms <= (ulong)raft_server::raft_limits_.response_limit_
                                  * ctx_->get_params()->heartbeat_interval_) {
    ...
    srv_to_join_ = new_ptr<peer,
                           ptr<srv_config>&,
                           context&,
                           timer_task<int32>::executor&,
                           ptr<logger>&>(conf_to_add_, *ctx_, exec, logger_);
...
void raft_server::invite_srv_to_join_cluster() {
    ...
    req->log_entries().push_back(
        new_ptr<log_entry>(state_->get_term(), c_conf->serialize(), log_val_type::conf));
...
ptr<resp_msg> raft_server::handle_join_cluster_req(req_msg& req) {
    ...
    ptr<cluster_config> c_config = cluster_config::deserialize(entries[0]->get_buff());
...
void raft_server::sync_log_to_new_srv(ulong start_idx) {
    ...
    if ((params->log_sync_stop_gap_ > 0 && gap < (ulong)params->log_sync_stop_gap_)
        ...
        ptr<cluster_config> new_conf =
            new_ptr<cluster_config>(log_store_->next_slot(), cur_conf->get_log_idx());
    ...
    if (/* start_idx > 0 && */ start_idx < log_store_->start_index()) {
        ...
        req = new_ptr<req_msg>(state_->get_term(),
                               ...
                               quick_commit_index_.load());
        req->log_entries().push_back(
            new_ptr<log_entry>(state_->get_term(), log_pack, log_val_type::log_pack));
    }
    ...
    if (!params->use_bg_thread_for_snapshot_io_) {
        ...
        snapshot_io_mgr::get_instance().invoke();
    }
}
...
ptr<resp_msg> raft_server::handle_log_sync_req(req_msg& req) {
    ...
    log_store_->apply_pack(req.get_last_log_idx() + 1, entries[0]->get_buff());
    p_db("last log %ld\n", log_store_->next_slot() - 1);
...
void raft_server::handle_log_sync_resp(resp_msg& resp) {
    if (srv_to_join_) {
        ...
        srv_to_join_->resume_heartbeat_speed();
...
ptr<resp_msg> raft_server::handle_rm_srv_req(req_msg& req) {
    std::vector<ptr<log_entry>>& entries = req.log_entries();
    ptr<resp_msg> resp = new_ptr<resp_msg>(
        state_->get_term(), msg_type::remove_server_response, id_, std::ref(leader_));

    if (entries.size() != 1 || entries[0]->get_buff().size() != size_int) {
    ...
    int32 srv_id = entries[0]->get_buff().get_int();
    if (srv_id == id_) {
    ...
    ptr<req_msg> leave_req(new_ptr<req_msg>(state_->get_term(),
...
void raft_server::rm_srv_from_cluster(int32 srv_id) {
    ...
    ptr<cluster_config> new_conf =
        new_ptr<cluster_config>(log_store_->next_slot(), cur_conf->get_log_idx());
    for (cluster_config::const_srv_iter it = cur_conf->get_servers().begin();
...
void raft_server::handle_join_leave_rpc_err(msg_type t_msg, ptr<peer> p) {
    if (t_msg == msg_type::leave_cluster_request) {
         ...
         */
        if (peers_.size() == 1) {
            auto pit = peers_.find(p->get_id());
            if (pit != peers_.end()) {
                pit->second->enable_heartbeat(false);
                peers_.erase(pit);
