...
void raft_server::enable_hb_for_peer(peer& p) {
    ...
    p_tr("peer %d, interval: %d\n", p.get_id(), p.get_current_heartbeat_interval());
    schedule_task(p.get_heartbeat_task(), p.get_current_heartbeat_interval());
}
...
void raft_server::handle_hb_timeout(int32 srv_id) {
    ...
    if (write_paused_ && reelaction_timer_.timeout()) {
        p_in("resign by timeout, %zu us elapsed, resign now", reelaction_timer_.get_us());
        leader_ = -1;
        become_follower();
    ...
    if (srv_to_join_snp_retry_required_ && srv_to_join_
        ...
        if (srv_to_join_->need_to_reconnect()) {
            ...
            if (resp_timer_ms >= (uint64_t)params->heartbeat_interval_
    ...
    if (p->is_leave_flag_set()) {
        ...
        p->inc_heartbeat_cnt_since_leave();
        int32 cur_cnt = p->get_heartbeat_cnt_since_leave();
    ...
    if (role_ == srv_role::leader) {
        ...
        request_append_entries(p);
        {
            ...
            if (p->is_heartbeat_enabled()) {
                // Schedule another heartbeat if heartbeat is still enabled
                schedule_task(p->get_heartbeat_task(),
                              p->get_current_heartbeat_interval());
            } else {
...
void raft_server::restart_election_timer() {
    ...
    if (election_task_) {
        ...
        election_task_ =
            new_ptr<timer_task<void>>(election_exec_, timer_task_type::election_timer);
    }
...
void raft_server::handle_election_timeout() {
    ...
    if (!im_learner_) {
        ...
        p_in("[ELECTION TIMEOUT] current role: %s, log last term %lu, "
             ...
             target_priority_,
             my_priority_,
             (heartbeat_alive_) ? "hb alive" : "hb dead",
...
void raft_server::cancel_schedulers() {
    ...
    for (auto it = peers_.begin(); it != peers_.end(); ++it) {
        ...
        if (p->get_heartbeat_task()) {
            cancel_task(p->get_heartbeat_task());
        }
