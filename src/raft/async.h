...
public:
  ...
  cmd_result()
      : err_(), code_(cmd_result_code::OK), has_result_(false), accepted_(false), handler_(nullptr),
        handler2_(nullptr) {}

  explicit cmd_result(T &result, cmd_result_code code = cmd_result_code::OK)
      : result_(result), err_(), code_(code), has_result_(true), accepted_(false), handler_(nullptr),
        handler2_(nullptr) {}

  explicit cmd_result(T &result, bool _accepted, cmd_result_code code = cmd_result_code::OK)
      : result_(result), err_(), code_(code), has_result_(true), accepted_(_accepted), handler_(nullptr),
        handler2_(nullptr) {}

  explicit cmd_result(const handler_type &handler)
      : err_(), code_(cmd_result_code::OK), has_result_(true), accepted_(false), handler_(handler), handler2_(nullptr) {
  }
