...
public:
  ...
  int32 get_leader() const {
    if (leader_ == id_ && role_ != srv_role::leader)
      return -1;
    return leader_;
  }
  ...
  static bool get_stat_histogram(const std::string &name, boost::unordered_map<double, uint64_t> &histogram_out);
...
public:
  ...
  void auto_fwd_resp_handler(ptr<cmd_result<ptr<buffer>>> presult, ptr<auto_fwd_pkg> cur_pkg, ptr<rpc_client> rpc_cli,
