...
snapshot_io_mgr::snapshot_io_mgr()
    : io_thread_event_awaiter_(new EventAwaiter())
    , terminating_(false) {
...
bool snapshot_io_mgr::push(ptr<snapshot_io_mgr::io_queue_elem>& elem) {
    ...
    logger* logger_ = elem->raft_->logger_.get();
...
void snapshot_io_mgr::invoke() { io_thread_event_awaiter_->invoke(); }

void snapshot_io_mgr::drop_reqs(raft_server* r) {
    auto_lock(queue_lock_);
    logger* logger_ = r->logger_.get();
...
void snapshot_io_mgr::shutdown() {
    ...
    if (io_thread_.joinable()) {
        io_thread_event_awaiter_->invoke();
        io_thread_.join();
    }
}
...
void snapshot_io_mgr::async_io_loop() {
    ...
    do {
        io_thread_event_awaiter_->wait_ms(1000);
        io_thread_event_awaiter_->reset();
        ...
        for (ptr<io_queue_elem>& elem: reqs) {
            ...
            if (elem->dst_->make_busy()) {
                ...
                elem->dst_->reset_last_sent_timer();
