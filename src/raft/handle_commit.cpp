...
#include <raft/handle_client_request.h>
...
void raft_server::commit(ulong target_idx) {
    if (target_idx > quick_commit_index_) {
        ...
        logging_sm_target_index_ = target_idx;
...
void raft_server::commit_app_log(ulong idx_to_commit,
    ...
    if (need_to_handle_commit_elem) {
        ...
        auto entry = coommit_ret_elems_.find(sm_idx);
        if (entry != coommit_ret_elems_.end()) {
        ...
        if (need_to_check_commit_ret) {
            ...
            coommit_ret_elems_.insert(std::make_pair(sm_idx, elem));
        }
    }
...
void raft_server::commit_conf(ulong idx_to_commit, ptr<log_entry>& le) {
    recur_lock(lock_);
    le->get_buff().pos(0);
    ptr<cluster_config> new_conf = cluster_config::deserialize(le->get_buff());
...
bool raft_server::snapshot_and_compact(ulong committed_idx, bool forced_creation) {
    ...
    if (params->enable_randomized_snap_creatioin_
    ...
    try {
        ...
        if ((forced_creation || !local_snp
            ...
            while (conf->get_log_idx() > committed_idx
                ...
                conf = cluster_config::deserialize(conf_log->get_buff());
            }
...
void raft_server::reconfigure(const ptr<cluster_config>& new_config) {
    ...
    for (std::list<ptr<srv_config>>::const_iterator it = new_srvs.begin();
         it != new_srvs.end();
         ++it) {
        auto pit = peers_.find((*it)->get_id());
    ...
    for (peer_iterator it = peers_.begin(); it != peers_.end(); ++it) {
    ...
    for (std::vector<ptr<srv_config>>::const_iterator it = srvs_added.begin();
        ...
        ptr<peer> p = new_ptr<peer,
                              ...
                              ptr<logger>&>(srv_added, *ctx_, exec, logger_);
    ...
    for (std::vector<int32>::const_iterator it = srvs_removed.begin();
        ...
        auto pit = peers_.find(srv_removed);
        if (pit != peers_.end()) {
            // WARNING:
...
bool raft_server::is_state_machine_paused() const {
