...
void peer::send_req(ptr<peer> self, ptr<req_msg> &req, rpc_handler &handler) {
  ...
  if (req) {
    p_tr("send req %d to %d, type %s", req->get_src(), req->get_dst(), msg_type_to_string(req->get_type()).c_str());
  }
  ...
  rpc_handler h = [this, self, rpc_local, &req, &pending](auto &&result, auto &&error) {
    this->handle_rpc_result(self, rpc_local, req, pending, std::forward<decltype(result)>(result),
...
void peer::handle_rpc_result(ptr<peer> myself, ptr<rpc_client> my_rpc_client, ptr<req_msg> &req,
                             ptr<rpc_result> &pending_result, ptr<resp_msg> &resp, ptr<rpc_exception> &exception) {
  boost::unordered_set<int> msg_types_to_free({msg_type::append_entries_request, msg_type::install_snapshot_request,
  ...
  if (req) {
    p_tr("resp of req %d -> %d, type %s, %s", req->get_src(), req->get_dst(),
         msg_type_to_string(req->get_type()).c_str(), (exception) ? exception->what() : "OK");
  }
