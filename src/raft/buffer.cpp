...
ulong buffer::get_ulong() {
    ...
    byte* d = data();
    ulong val = 0L;
    for (size_t i = 0; i < size_ulong; ++i) {
        ulong byte_val = (ulong) * (d + i);
        val += (byte_val << (i * 8));
    }

    __mv_fw_block(this, size_ulong);
    return val;
}
