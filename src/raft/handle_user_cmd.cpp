...
ptr<cmd_result<ptr<buffer>>> raft_server::add_srv(const srv_config& srv) {
    ...
    ptr<log_entry> log(new_ptr<log_entry>(0, buf, log_val_type::cluster_server));
...
ptr<cmd_result<ptr<buffer>>> raft_server::remove_srv(const int srv_id) {
    ptr<buffer> buf(buffer::alloc(size_int));
    buf->put(srv_id);
    buf->pos(0);
    ptr<log_entry> log(new_ptr<log_entry>(0, buf, log_val_type::cluster_server));
    ptr<req_msg> req = new_ptr<req_msg>(
...
raft_server::append_entries_ext(const std::vector<ptr<buffer>>& logs,
    ...
    if (logs.size() == 0) {
        ...
        return new_ptr<cmd_result<ptr<buffer>>>(result);
    }
    ...
    for (auto it = logs.begin(); it != logs.end(); ++it) {
        ...
        buf->pos(0);
        ptr<log_entry> log(new_ptr<log_entry>(0, *it, log_val_type::app_log));
        req->log_entries().push_back(log);
    }
...
raft_server::send_msg_to_leader(ptr<req_msg>& req, const req_ext_params& ext_params) {
    ...
    if (leader_id == -1) {
        ...
        ptr<cmd_result<ptr<buffer>>> ret =
            new_ptr<cmd_result<ptr<buffer>>>(result, cmd_result_code::NOT_LEADER);
        return ret;
    }
    ...
    if (leader_id == id_) {
        ...
        if (!resp) {
            ...
            ptr<cmd_result<ptr<buffer>>> ret =
                new_ptr<cmd_result<ptr<buffer>>>(result, cmd_result_code::BAD_REQUEST);
            return ret;
        }
        ...
        if (!ret) {
            ...
            ret = new_ptr<cmd_result<ptr<buffer>>>(
    ...
    bool is_blocking_mode = (params->return_method_type_ == raft_params::blocking);
        ...
        if (e_rpc == cur_pkg->rpc_client_idle_.end()) {
            ...
            if (cur_pkg->rpc_client_idle_.size() + cur_pkg->rpc_client_in_use_.size()
                ...
                if (is_blocking_mode) {
                    ...
                    req_resp_pair.resp = new_ptr<cmd_result<ptr<buffer>>>();
    ...
    ptr<cmd_result<ptr<buffer>>> presult(new_ptr<cmd_result<ptr<buffer>>>());
    ...
    if (params->return_method_type_ == raft_params::blocking) {
        presult->get();
    }
...
void raft_server::auto_fwd_release_rpc_cli(ptr<auto_fwd_pkg> cur_pkg,
    ...
    if (is_blocking_mode) {
        ...
        std::unique_lock<std::mutex> ll(auto_fwg_reqs_lock_);
        if (!auto_fwd_reqs_.empty()) {
            ...
            rpc_handler handler =
                [this, capture0 = entry.resp, cur_pkg, rpc_cli](auto&& PH1, auto&& PH2) {
                    auto_fwd_resp_handler(capture0,
                                          cur_pkg,
                                          rpc_cli,
