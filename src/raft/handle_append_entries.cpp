...
#include "logger.h"
#include "peer.h"
#include "raft/raft_server_handler.h"
#include "raft/snapshot.h"
#include "raft/stat_mgr.h"
#include "resp_msg.h"
#include "snapshot.h"
#include "tracer.h"
namespace raft {
...
bool raft_server::request_append_entries(ptr<peer> p) {
    ...
    if (params->auto_adjust_quorum_for_smaller_cluster_ && get_num_voting_members() == 2
    ...
    if (last_active_time_ms
        > params->heartbeat_interval_ * raft_server::raft_limits_.reconnect_limit_) {
    ...
    if (params->use_bg_thread_for_snapshot_io_) {
        ...
        if (snapshot_io_mgr::get_instance().has_pending_request(this, p->get_id())) {
    ...
    if (p->make_busy()) {
        ...
        if (!p->is_manual_free()) {
            // Actual recovery.
            if (p->get_long_pause_warnings()
                >= raft_server::raft_limits_.warning_limit_) {
                int32 last_ts_ms = p->get_last_sent_timer_us() / 1000;
                p->inc_recovery_cnt();
                p_wn("recovered from long pause to peer %d, %d warnings, "
                     "%d ms, %d times",
                     p->get_id(),
                     p->get_long_pause_warnings(),
                     last_ts_ms,
                     p->get_recovery_cnt());
...
ptr<resp_msg> raft_server::handle_append_entries(req_msg& req) {
    ...
    if (req.log_entries().size() > 0) {
        ...
        ptr<raft_params> params = ctx_->get_params();
        if (params->parrallel_log_appending_) {
            ...
            while (last_durable_index
                ...
                p_tr("durable idnex %lu, sleep and wait for log appending completion",
                     last_durable_index);
                ea_follower_log_append_->wait_ms(params->heartbeat_interval_);
    ...
    if (time_ms >= ctx_->get_params()->heartbeat_interval_) {
...
ulong raft_server::get_expected_committed_log_idx() {
    ...
    if (ctx_->get_params()->use_full_comsensus_among_healthy_members_) {
