...
#include <mutex>
#include <string>
constexpr static std::array<std::string, 6> GroupLockModeStr = {"NON_LOCK", "IS", "IX", "S", "X", "SIX"};
class LockManager {
  ...
  class LockRequest {
  public:
    LockRequest(txn_id_t txn_id, LockMode lock_mode, timestamp_t txn_ts)
        : txn_id_(txn_id), lock_mode_(lock_mode), granted_(false), txn_ts_(txn_ts) {}

    txn_id_t txn_id_;    // 申请加锁的事务ID
    LockMode lock_mode_; // 事务申请加锁的类型
    bool granted_;       // 该事务是否已经被赋予锁
    timestamp_t txn_ts_; // 申请加锁事务的开始时间戳
  };
