...
public:
    ...
    inline std::shared_ptr<std::deque<TableWriteRecord*>> get_table_write_set() {
    ...
    inline void append_index_write_record(IndexWriteRecord* write_record) {
...
private:
    ...
    // 事务包含的所有写操作
    std::shared_ptr<std::deque<TableWriteRecord*>>
        table_write_set_; // 事务包含的table所有写操作
    std::shared_ptr<std::deque<IndexWriteRecord*>>
        index_write_set_; // 事务包含的index所有写操作
