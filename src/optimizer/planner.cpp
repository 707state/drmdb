...
std::pair<bool, IndexMeta>
Planner::get_index_cols(std::string tab_name,
                        std::vector<Condition> curr_conds,
                        std::vector<std::string>& index_col_names) {
    // bool Planner::get_index_cols(std::string tab_name, std::vector<Condition>
    // curr_conds, std::vector<std::string>& index_col_names) {
...
std::shared_ptr<Plan> Planner::make_one_rel(std::shared_ptr<Query> query) {
    ...
    std::vector<Condition> having_clauses = query->having_clauses;
    ...
    for (size_t i = 0; i < tables.size(); i++) {
        ...
        std::vector<std::string> index_col_names;
        // bool index_exist = get_index_cols(tables[i], curr_conds,
        // index_col_names);
        auto [index_exist, index_meta] =
            get_index_cols(tables[i], curr_conds, index_col_names);
        if (index_exist == false) { // 该表没有索引
            index_col_names.clear();
            table_scan_executors[i] =
                // std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, tables[i],
                // curr_conds, index_col_names);
                std::make_shared<ScanPlan>(T_SeqScan,
                                           sm_manager_.get(),
                                           tables[i],
                                           curr_conds,
                                           index_col_names,
            ...
            //     curr_conds, index_col_names);
            // auto index_scan = std::make_shared<ScanPlan>(T_IndexScan, sm_manager_,
            // tables[i], curr_conds, index_col_names);
            auto index_scan = std::make_shared<ScanPlan>(T_IndexScan,
                                                         sm_manager_.get(),
                                                         tables[i],
                                                         curr_conds,
                                                         index_col_names,
    ...
    if (conds.size() >= 1) {
        ...
        while (it != conds.end()) {
            ...
            if (std::find(
                    joined_tables.begin(), joined_tables.end(), it->lhs_col.tab_name)
                == joined_tables.end()) {
                left_need_to_join_executors = pop_scan(
                    scantbl, it->lhs_col.tab_name, joined_tables, table_scan_executors);
            }
            if (std::find(
                    joined_tables.begin(), joined_tables.end(), it->rhs_col.tab_name)
                == joined_tables.end()) {
            ...
            if (left_need_to_join_executors != nullptr
                ...
                std::shared_ptr<Plan> temp_join_executors =
                    std::make_shared<JoinPlan>(T_NestLoop,
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query> query,
    ...
    if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
        ...
        // load;
        plannerRoot = std::make_shared<DMLPlan>(T_Load,
                                                std::shared_ptr<Plan>(),
                                                x->tab_name_,
                                                query->values,
                                                std::vector<Condition>(),
        ...
        std::vector<std::string> index_col_names;
        // bool index_exist = get_index_cols(x->tab_name, query->conds,
        // index_col_names);
        auto [index_exist, index_meta] =
        ...
        if (index_exist == false) { // 该表没有索引
            ...
            table_scan_executors = std::make_shared<ScanPlan>(
                T_SeqScan, sm_manager_.get(), x->tab_name, query->conds, index_col_names);
        } else { // 存在索引
            // table_scan_executors =
            //     std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, x->tab_name,
        ...
        plannerRoot = std::make_shared<DMLPlan>(T_Delete,
                                                table_scan_executors,
                                                x->tab_name,
                                                std::vector<Value>(),
                                                query->conds,
        ...
        plannerRoot = std::make_shared<DMLPlan>(T_Update,
                                                table_scan_executors,
                                                x->tab_name,
                                                std::vector<Value>(),
                                                query->conds,
        ...
        auto aggre_plan = std::make_shared<DMLPlan>(T_select,
                                                    projection,
                                                    std::string(),
                                                    std::vector<Value>(),
                                                    std::vector<Condition>(),
                                                    std::vector<SetClause>());
        aggre_plan->having_clauses_ = query->having_clauses;
        aggre_plan->group_by_cols_ = query->group_by_cols;
