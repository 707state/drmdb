...
std::pair<bool, IndexMeta> Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds,
  ...
  for (size_t i = 0; i < tables.size(); i++) {
    ...
    if (index_exist == false) { // 该表没有索引
      ...
      table_scan_executors[i] =
          ...
          std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, tables[i], curr_conds, index_col_names, having_clauses);
    } else { // 存在索引
      // table_scan_executors[i] =
      ...
      auto index_scan =
          std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, tables[i], curr_conds, index_col_names, having_clauses);
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query> query, Context *context) {
  ...
  if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
    ...
    // load;
    plannerRoot = std::make_shared<DMLPlan>(T_Load, std::shared_ptr<Plan>(), x->tab_name_, query->values,
                                            std::vector<Condition>(), std::vector<SetClause>());
    ...
    auto aggre_plan = std::make_shared<DMLPlan>(T_select, projection, std::string(), std::vector<Value>(),
                                                std::vector<Condition>(), std::vector<SetClause>());
