...
public:
  SeqScanExecutor(std::shared_ptr<SmManager> sm_manager, std::string tab_name, std::vector<Condition> conds,
                  std::shared_ptr<Context> context, std::vector<Condition> having_clauses)
      : conds_(std::move(conds)), fh_(sm_manager_->fhs_.at(tab_name_).get()),
        len_(cols_.back().offset + cols_.back().len), fed_conds_(conds_), isend(false) {
