...
public:
    ...
    ExecutorJoiningBuffer(std::unique_ptr<AbstractExecutor>* prev)
        : prev_(prev) {
    ...
    bool is_end() { return buffer_->size_ == 0; }
};

class BlockNestedLoopJoinExecutor : public AbstractExecutor,
                                    public ConditionDependedExecutor {
private:
...
public:
    ...
    std::unique_ptr<RmRecord> Next() override {
        ...
        memcpy(ret->data + (*left_record)->size,
               (*right_record)->data,
               (*right_record)->size);
        return ret;
    }
                    ...
                    while (!(*right_join_buffer_)->is_end()) {
                        ...
                        for (auto cond: conds_) {
                            ...
                            if (cond.is_rhs_val) {
                                ...
                                auto right_cols = right_->cols();
                                auto right_col =
                                    *(right_->get_col(right_cols, cond.rhs_col));
