...
UpdateExecutor::UpdateExecutor(std::shared_ptr<SmManager> sm_manager, const std::string &tab_name,
                               std::vector<SetClause> set_clauses, std::vector<Condition> conds, std::vector<Rid> rids,
...
auto UpdateExecutor::cols() const -> const std::vector<ColMeta> & { return tab_.cols; }
  ...
  for (auto &rid : rids_) {
    ...
    try {
      for (auto &index : tab_.indexes) {
        ...
        /* 创建并添加索引写入记录到事务中 */
        auto index_rcd =
            std::make_shared<IndexWriteRecord>(WType::DELETE_TUPLE, tab_name_, rid, key, index.col_tot_len);
      ...
      /* 更新文件中的记录 */
      fh_->update_record(rid, new_record.get()->data, context_.get());
      APPEND_TO_FILE(rid.page_no, rid.slot_no, "WRITE");

      /* 创建并添加表写入记录到事务中 */
      auto write_rcd = std::make_shared<TableWriteRecord>(WType::UPDATE_TUPLE, tab_name_, rid, *record);
      ...
      for (size_t i = 0; i < tab_.indexes.size(); ++i) {
        ...
        /* 创建并添加索引写入记录到事务中 */
        auto index_rcd =
            std::make_shared<IndexWriteRecord>(WType::INSERT_TUPLE, tab_name_, rid, key, index.col_tot_len);
        context_->txn_->append_index_write_record(index_rcd);
        delete[] key;
      }
      ...
      /* 恢复record */
      fh_->update_record(rid, record->data, context_.get());
      /* 恢复所有的index */
      for (size_t i = 0; i < tab_.indexes.size(); ++i) {
        auto &index = tab_.indexes[i];
        auto ih = sm_manager_->ihs_.at(sm_manager_->get_ix_manager()->get_index_name(tab_name_, index.cols)).get();
