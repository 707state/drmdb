...
LoadExecutor::LoadExecutor(std::shared_ptr<SmManager> sm_manager, const std::string &tab_name,
                           std::vector<Value> values, std::shared_ptr<Context> context)
    : file_path_(values.at(0).str_val), fh_(sm_manager->fhs_.at(tab_name).get()) {
...
auto LoadExecutor::split(std::vector<std::string_view> &split_buf, std::string &origin_str, char delim) -> void {
  split_buf.clear();
  int last_index = 0;
  ...
  split_buf.emplace_back(origin_str.data() + last_index, origin_str.length() - last_index);
}
...
void *LoadExecutor::load_task(void *param) {
  ...
  if (!indexes.empty()) {
    // 调用sort命令排序，我真是个天才
    std::string sort_prog = "LC_ALL=C sort -n -t , --parallel=8 -o " + param_->tab_name + "_sorted.csv";
    // 所有索引都要重建
    for (auto &index : indexes) {
      ...
      int val_int_buf = 0;
      float val_float_buf = NAN;
      std::string val_str_buf;
      DateTime val_datetime_buf{};
      uint64_t val_datetime_raw_buf = 0;

      auto index_handle = param_->sm_manager->ihs_
                              .at(param_->sm_manager->get_ix_manager()->get_index_name(param_->tab_name, index.cols))
                              .get();
      ...
      while (!csv_file_input.eof()) {
        ...
        auto this_record_addr = this_bitmap_addr + rm_file_hdr.bitmap_size + rm_slot_index * rm_file_hdr.record_size;
        ...
        for (int i = 0; i < tab_cols.size(); ++i) {
          ...
          case TYPE_INT:
            ...
            break;
          case TYPE_STRING:
            std::memcpy(this_record_addr + col.offset, split_buf[i].data(), split_buf[i].size());
            break;
          case TYPE_FLOAT:
            val_str_buf = split_buf[i];
          ...
          case TYPE_DATETIME:
            ...
            std::memcpy(this_record_addr + col.offset, &val_datetime_raw_buf, col.len);
            break;
          }
        }
        ...
        if (rm_slot_index == rm_file_hdr.num_records_per_page) {
          ...
          if (page_buffer_index == kPageBufferSize) {
            ...
            param_->sm_manager->get_disk_manager()->write_full_pages(
                param_->fh->GetFd(), rm_page_index - kPageBufferSize, page_buf, kPageBufferSize);
            page_buffer_index = 0;
      ...
      if (page_buffer_index != 0 || rm_slot_index != 0) {
        ...
        param_->sm_manager->get_disk_manager()->write_full_pages(
            param_->fh->GetFd(), (rm_page_index - 1) / kPageBufferSize * kPageBufferSize + 1, page_buf,
            page_buffer_index + 1);
    ...
    while (!csv_file_input.eof()) {
      ...
      if (rm_slot_index == rm_file_hdr.num_records_per_page) {
        ...
        if (page_buffer_index == kPageBufferSize) {
          // 页缓冲满了, flush
          param_->sm_manager->get_disk_manager()->write_full_pages(param_->fh->GetFd(), rm_page_index - kPageBufferSize,
                                                                   page_buf, kPageBufferSize);
          param_->fh->get_file_hdr_ptr()->num_pages += kPageBufferSize;
          page_buffer_index = 0;
        }
        this_page_addr = page_buf + page_buffer_index * PAGE_SIZE;
