    ...
    void nextTuple() override {
        while (true) {
            ...
            for (auto& cond: fed_conds_) {
                ...
                Value left_val =
                    get_record_value(*lrec.get(), left_off, left_len, left_type);
                Value right_val =
                    get_record_value(*rrec.get(), right_off, right_len, right_type);
            ...
            if (col.offset < left_->tupleLen()) {
                ...
                for (auto& c: col_meta) {
                    if (c.name.compare(col.name) == 0) {
                        new_rec.set_column_value(
                            col.offset, col.len, lrec->get_column_value(c.offset, c.len));
                        break;
                    }
                }
                ...
                for (auto& c: col_meta) {
                    if (c.name.compare(col.name) == 0) {
                        ...
                        new_rec.set_column_value(
                            col.offset, col.len, rrec->get_column_value(c.offset, c.len));
                        break;
                    }
                }
            }
