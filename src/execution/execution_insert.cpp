...
InsertExecutor::InsertExecutor(std::shared_ptr<SmManager> sm_manager, const std::string &tab_name,
                               std::vector<Value> values, std::shared_ptr<Context> context)
    : tab_(sm_manager_->db_.get_table(tab_name)), values_(values) {
...
auto InsertExecutor::Next() -> std::unique_ptr<RmRecord> {
  ...
  rid_ = fh_->insert_record(rec.data, context_.get());
  context_->txn_->append_table_write_record(std::make_shared<TableWriteRecord>(WType::INSERT_TUPLE, tab_name_, rid_));
  ...
  try {
    ...
    for (size_t i = 0; i < tab_.indexes.size(); ++i) {
      ...
      auto index_rcd = std::make_shared<IndexWriteRecord>(WType::INSERT_TUPLE, tab_name_, rid_, key, index.col_tot_len);
      context_->txn_->append_index_write_record(index_rcd);
      delete[] key;
    }
  } catch (InternalError &error) {
    fh_->delete_record(rid_, context_.get());
