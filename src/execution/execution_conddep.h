...
public:
  ...
  ConditionDependedExecutor(std::string tab_name, std::shared_ptr<SmManager> sm_manager)
      : tab_name_(tab_name), sm_manager_(sm_manager) {}

  static Value get_record_value(const RmRecord &record, int offset, int len, ColType type) {
    Value val;
    val.type = type;

    if (type == TYPE_INT) {
      val.set_int(*reinterpret_cast<int *>(record.data + offset));
    } else if (type == TYPE_FLOAT) {
      val.set_float(*reinterpret_cast<float *>(record.data + offset));
    } else if (type == TYPE_STRING) {
      val.set_str(std::string(record.data + offset, len));
    } else if (type == TYPE_DATETIME) {
      uint64_t tmp = 0;
      memcpy(reinterpret_cast<char *>(&tmp), record.data + offset, len);
  ...
  Value get_record_value(const RmRecord &record, const TabCol &col) {
    ...
    if (col_meta.type == TYPE_INT) {
      ...
      val.set_float(*reinterpret_cast<float *>(record.data + col_meta.offset));
      ...
      uint64_t tmp = 0;
      memcpy(reinterpret_cast<char *>(&tmp), record.data + col_meta.offset, col_meta.len);
      val.set_datetime(tmp);
    } else {
      throw InvalidTypeError();
    }
  ...
  // 从Record中取出某一列的Value
  static Value get_record_value(const std::unique_ptr<RmRecord> &record, const ColMeta &col_meta) {
    Value val;
    val.type = col_meta.type;

    if (col_meta.type == TYPE_INT) {
      val.set_int(*reinterpret_cast<int *>(record->data + col_meta.offset));
    } else if (col_meta.type == TYPE_FLOAT) {
      val.set_float(*reinterpret_cast<float *>(record->data + col_meta.offset));
    } else if (col_meta.type == TYPE_STRING) {
      int offset = col_meta.offset;
  ...
  static bool check_cond(Value left, Value right, CompOp op) {
    ...
    if (left.type == TYPE_INT && right.type == TYPE_INT) {
      ...
      cp_res = left.str_val.substr(0, left.str_val.find_first_of((char)0)).compare(right.str_val);
      // 2024.8.7修改
      ...
      //      cp_res = std::strncmp(left.str_val.c_str(), right.str_val.c_str(), min_len);
      //      if (cp_res == 0) {
      //        cp_res = (left_len == right_len) ? 0 : (left_len < right_len ? -1 : 1);
      //      }

    } else if (left.type == TYPE_DATETIME && right.type == TYPE_DATETIME) {
      // std::cout<<"left.datetime_val: "<<left.datetime_val.encode_to_string()<<" right.datetime_val:
      // "<<right.datetime_val.encode_to_string();
  ...
  bool check_cond(Value left, Value right, Condition cond) { return check_cond(left, right, cond.op); }
  ...
  std::shared_ptr<SmManager> get_sm_manager() { return sm_manager_; }
