...
void QlManager::aggregate_select_from(std::unique_ptr<AbstractExecutor> executorTreeRoot, std::vector<TabCol> sel_cols,
                                      Context *context, std::vector<Condition> having_clauses,
                                      std::vector<TabCol> group_by_cols) {
  std::cout << "[Internal] Aggregate Select" << std::endl;
  for (auto &having_clause : having_clauses) {
    if (having_clause.lhs_col.col_name.empty() && having_clause.lhs_col.ag_type == ast::SV_AGGRE_COUNT) {
      // count(*) 的情况
  ...
  for (auto &sel_col : sel_cols) {
    ...
    captions.emplace_back(sel_col.ag_type == ast::SV_AGGRE_NONE ? sel_col.col_name : sel_col.as_name);
  }
  ...
  for (executorTreeRoot->beginTuple(); !executorTreeRoot->is_end(); executorTreeRoot->nextTuple()) {
    ...
    for (auto &having_clause : having_clauses) {
      ...
      auto it = std::find_if(sel_cols.begin(), sel_cols.end(), [&](const TabCol &existing_col) {
        return existing_col.col_name == col.col_name && existing_col.ag_type == col.ag_type;
      });
      if (it == sel_cols.end()) {
    ...
    for (auto &col : executorTreeRoot->cols()) {
      ...
      if (col.type == TYPE_INT) {
        ...
        if (std::find_if(group_by_cols.begin(), group_by_cols.end(),
                         [&col](const TabCol &g_col) { return col.name == g_col.col_name; }) != group_by_cols.end()) {
      ...
      for (size_t i = 0; i < sel_cols.size(); ++i) {
        try {
          ...
          if (sel_cols[i].ag_type == ast::SV_AGGRE_MAX) {
            ...
            // 检查是否有小数点来判断是什么数
            bool isFloatResult = result.values[i].find('.') != std::string::npos;
            ...
            if (!isFloatResult && !isFloatNew) {
              ...
              result.values[i] = std::to_string(intValueResult + intValueNew);
            } else {
              // 反之同理
              float floatValueResult = std::stof(result.values[i]);
              float floatValueNew = std::stof(agg_results[i]);
              result.values[i] = std::to_string(floatValueResult + floatValueNew);
            }
          } else if (sel_cols[i].ag_type == ast::SV_AGGRE_COUNT) {
            // count的无所谓
            result.values[i] = std::to_string(std::stoi(result.values[i]) + 1);
  ...
  for (const auto &[group, result] : agg_values) {
    ...
    for (const auto &having_clause : having_clauses) {
      ...
      size_t index = std::distance(
          sel_cols.begin(), std::find_if(sel_cols.begin(), sel_cols.end(), [&having_clause](const TabCol &s_col) {
            return s_col.col_name == having_clause.lhs_col.col_name && s_col.ag_type == having_clause.lhs_col.ag_type;
          }));
            ...
            std::find_if(result.values.begin(), result.values.end(),
                         [&having_clause, &sel_cols, &result](const std::string &val) {
                           ...
                           size_t potential_index = std::distance(
                               sel_cols.begin(),
                               std::find_if(sel_cols.begin(), sel_cols.end(), [&having_clause](const TabCol &s_col) {
                                 return s_col.col_name == having_clause.lhs_col.col_name;
                               }));
                           return potential_index != sel_cols.size() && potential_index < result.values.size();
                         }));
      ...
      if (having_clause.is_rhs_val) {
        ...
        size_t other_index = std::distance(
            sel_cols.begin(), std::find_if(sel_cols.begin(), sel_cols.end(), [&having_clause](const TabCol &s_col) {
              return s_col.col_name == having_clause.rhs_col.col_name;
            }));
    ...
    if (pass_having) {
      ...
      for (const auto &sel_col : sel_cols) {
        if (sel_col.ag_type == ast::SV_AGGRE_NONE) {
          ...
          auto it = std::find_if(group_by_cols.begin(), group_by_cols.end(),
                                 [&sel_col](const TabCol &g_col) { return g_col.col_name == sel_col.col_name; });
          ...
          // 如果聚合类型是count，特殊处理
          size_t index = std::distance(
              sel_cols.begin(), std::find_if(sel_cols.begin(), sel_cols.end(), [&sel_col](const TabCol &s_col) {
                return s_col.col_name == sel_col.col_name && s_col.ag_type == ast::SV_AGGRE_COUNT;
              }));
          if (index == sel_cols.size()) {
          ...
          size_t index = std::distance(sel_cols.begin(),
                                       std::find_if(sel_cols.begin(), sel_cols.end(), [&sel_col](const TabCol &s_col) {
...
void QlManager::aggregate_select_from_with_index(std::unique_ptr<AbstractExecutor> executorTreeRoot,
...
void QlManager::fast_aggre_with_index(std::vector<TabCol> sel_cols, IxIndexHandle *ix_index_handle,
                                      IndexMeta index_meta, std::vector<Condition> conds, RmFileHandle *rm_file_handle,
                                      Context *context) {
  // 处理表头
  std::vector<std::string> captions;
  captions.reserve(sel_cols.size());
  ...
  for (auto &col : index_meta.cols) {
    ...
    for (auto &cond : conds) {
      if (cond.lhs_col.col_name == col.name && cond.is_rhs_val) {
        switch (cond.op) {
        case OP_EQ: {
          if (ConditionDependedExecutor::check_cond(cond.rhs_val, lower_value, OP_GT)) {
            lower_value = cond.rhs_val;
          }
          if (ConditionDependedExecutor::check_cond(cond.rhs_val, upper_value, OP_LT)) {
  ...
  if (sel_cols[0].ag_type == ast::SV_AGGRE_COUNT) {
    ...
    auto col_def = *(sm_manager_->db_.get_table(sel_cols[0].tab_name).get_col(sel_cols[0].col_name));
