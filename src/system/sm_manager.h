...
public:
  ...
  bool enable_output_ =
      true; // 管理是否允许输出到output.txt中, true为允许输出, false为不允许输出, 即执行结果不会出现在output.txt中
private:
...
public:
  SmManager(std::shared_ptr<DiskManager> disk_manager, std::shared_ptr<BufferPoolManager> buffer_pool_manager,
            std::shared_ptr<RmManager> rm_manager, std::shared_ptr<IxManager> ix_manager)
      : disk_manager_(std::move(disk_manager)), buffer_pool_manager_(std::move(buffer_pool_manager)),
        rm_manager_(std::move(rm_manager)), ix_manager_(std::move(ix_manager)) {}
  ...
  void reset_index(const std::string &tab_name, const std::vector<ColMeta> &col_names, Context *context);
};
