...
void SmManager::open_db(const std::string &db_name) {
  ...
  for (auto &entry : db_.tabs_) {
    ...
    for (auto &ix_meta : tab.indexes) {
      ihs_.emplace(ix_manager_->get_index_name(tab.name, ix_meta.cols),
                   ix_manager_->open_index(tab.name, ix_meta.cols));
    }
  }
}
...
void SmManager::create_table(const std::string &tab_name, const std::vector<ColDef> &col_defs, Context *context) {
  if (db_.is_table(tab_name)) {
    throw TableExisisError(tab_name);
  }
  // Create table meta
  int curr_offset = 0;
  TabMeta tab;
  ...
  fhs_.insert(std::make_pair(tab_name, rm_manager_->open_file(tab_name)));
  //  fhs_.emplace(tab_name, rm_manager_->open_file(tab_name));

  if (context != nullptr) {
    context->lock_mgr_->lock_exclusive_on_table(context->txn_.get(), fhs_[tab_name]->GetFd());
  }
...
void SmManager::drop_table(const std::string &tab_name, Context *context) {
  ...
  for (auto it = db_.tabs_[tab_name].indexes.begin(); it != db_.tabs_[tab_name].indexes.end(); it++) {
    ...
    // ix_manager_->destroy_index(ihs_.at(index_name).get(), tab_name,
    // col_names);
...
void SmManager::create_index(const std::string &tab_name, const std::vector<std::string> &col_names, Context *context) {
  if (context != nullptr) {
    context->lock_mgr_->lock_shared_on_table(context->txn_.get(), fhs_[tab_name]->GetFd());
  }
  ...
  // 检查索引存在性
  if (ix_manager_->index_exists(tab_name, col_names)) {
  ...
  // 读取索引（索引文件头之后的部分）
  ihs_.insert(std::make_pair(index_name, ix_manager_->open_index(tab_name, col_names)));
  auto ix_hdl = ihs_.at(index_name).get();

  auto file_hdl = fhs_.at(tab_name);
  ...
  try {
    for (RmScan rm_scan(file_hdl); !rm_scan.is_end(); rm_scan.next()) {
      ...
      ix_make_key(key, *rec, ix_meta);
      ix_hdl->insert_entry(key, rm_scan.rid(), context->txn_.get());
    }
...
void SmManager::drop_index(const std::string &tab_name, const std::vector<std::string> &col_names, Context *context) {
  ...
  // // 删除TabMeta中的索引
  // for (auto it = db_.get_table(tab_name).indexes.begin(); it !=
  // db_.get_table(tab_name).indexes.end(); it++) {
...
void SmManager::reset_index(const std::string &tab_name, const std::vector<ColMeta> &col_names, Context *context) {
  drop_index(tab_name, col_names, context);
  if (context != nullptr) {
    context->lock_mgr_->lock_shared_on_table(context->txn_.get(), fhs_[tab_name]->GetFd());
  }

  const std::string &ix_file_name = tab_name;
  std::vector<ColMeta> cols;
  IndexMeta ix_meta;
  ix_meta.tab_name = tab_name;
  ix_meta.col_tot_len = 0;
  ...
  auto &tab = db_.get_table(tab_name);
  for (const auto &col_name : col_names) {
    ColMeta &col = *tab.get_col(col_name.name);
    col.index = true;
    ix_meta.col_tot_len += col.len;
    ix_meta.cols.push_back(col);
    cols.push_back(col);
