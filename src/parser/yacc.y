...
// keywords
%token SHOW TABLES CREATE TABLE DROP DESC INSERT LOAD INTO VALUES DELETE FROM ASC ORDER BY GROUP HAVING
WHERE UPDATE SET SELECT INT CHAR FLOAT DATETIME INDEX AND JOIN EXIT HELP TXN_BEGIN TXN_COMMIT TXN_ABORT TXN_ROLLBACK ORDER_BY LIMIT AS
SUM COUNT MAX MIN OUTPUT_FILE OFF
// non-keywords
%token LEQ NEQ GEQ T_EOF

// type-specific tokens
%token <sv_str> LOAD_PATH
...
%type <sv_str> tbName colName asName optAsClause
...
%type <sv_ag_type> AGGREGATOR
// %type <sv_ag_type> AGGRE_SUM AGGRE_COUNT AGGRE_MAX AGGRE_MIN
%type <sv_cols> groupByClause optGroupByClause
%type <sv_conds> havingClause optHavingClause
...
dml:
    ...
    |   SELECT selector FROM tableList optWhereClause optGroupByClause optHavingClause opt_order_clauses
    {
...
// tbName, colName, Aggre, asName
col:
        tbName '.' colName optAsClause
    {
        $$ = std::make_shared<Col>($1, $3, SV_AGGRE_NONE, $4);
    }
    |   colName optAsClause
    {
        $$ = std::make_shared<Col>("", $1, SV_AGGRE_NONE, $2);
    }
    |   AGGREGATOR '(' tbName '.' colName ')' optAsClause
    {
        $$ = std::make_shared<Col>($3, $5, $1, $7);
    }
    |   AGGREGATOR '(' '*' ')' optAsClause
    {
        $$ = std::make_shared<Col>("", "", $1, $5);
    }
    |   AGGREGATOR '(' colName ')' optAsClause
    {
