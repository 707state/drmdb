...
private:
  ...
  static std::string type2str(SvType type) {
    static boost::unordered_map<SvType, std::string> m{
        {SV_TYPE_INT, "INT"}, {SV_TYPE_FLOAT, "FLOAT"}, {SV_TYPE_STRING, "STRING"}, {SV_TYPE_DATETIME, "DATETIME"}};
    return m.at(type);
  }

  static std::string op2str(SvCompOp op) {
    static boost::unordered_map<SvCompOp, std::string> m{
  ...
  static void print_node(const std::shared_ptr<TreeNode> &node, int offset) {
    ...
    if (auto x = std::dynamic_pointer_cast<Help>(node)) {
      ...
      std::cout << "LOAD\n";
      print_val(x->file_path_, offset);
      print_val(x->tab_name_, offset);
