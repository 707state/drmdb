...
bool is_run_command(std::string &command) {
  return command.substr(0, 3) == "run";
}
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (!command.empty()) {
      ...
      if (is_run_command(command)) {
        // auto run sql
        printf("# Auto Run Begin\n");
        std::string sql_path = command.substr(4, command.length() - 4);
        ...
        std::string cur_sql_buffer;
        while (!sql_stream.eof()) {
          std::getline(sql_stream, cur_sql_buffer);
          if (!cur_sql_buffer.empty()) {
            printf("# Run SQL: '%s'\n", cur_sql_buffer.c_str());
            if ((send_bytes = write(sockfd, cur_sql_buffer.c_str(), cur_sql_buffer.length() + 1)) ==
                -1) {
